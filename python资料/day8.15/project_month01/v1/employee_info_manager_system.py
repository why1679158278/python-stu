...
class EmployeeController:
    ...
    # __start_id = 1000  # 如果有多个员工控制器,start_id不重复
    #
    # @classmethod
    # def set_Employee_id(cls,stu_target):
    ...
    #     self.__list_Employees = []
    #
    # def add_Employee(self, stu_target):
    #     EmployeeController.set_Employee_id(stu_target)
    #     self.__list_Employees.append(stu_target)
    def remove_employee(self, eid):
        ...
        :return: 是否移除成功
        """
        for i in range(len(self.__list_employees)):
            if self.__list_employees[i].eid == eid:
                del self.__list_employees[i]
                return True  # 删除成功
        return False  # 删除失败
